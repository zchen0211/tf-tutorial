...
for i in range(sample_step):
  ...
  for sgd_i in range(sgd_step):
    ...
    # raw loss
    loss = 0.5 * np.mean(np.square(y1 - np.matmul(Wf, np.matmul(Ws, xnp1))))
    print 'raw loss: ', loss
    ...
    loss = 0.5 * np.mean(np.square(y2 - np.matmul(Wf, np.matmul(Ws, xnp2))))
    print 'raw loss before slow update: ', loss
    print 'current fast learner: ', Wf 
    print ' '

    grad_Ws = lr_fast * np.matmul(xnp1, np.transpose(y1)) / float(batch_size)
    grad_Ws = np.matmul(grad_Ws, delta_y2)
    grad_Ws = np.matmul(grad_Ws, np.transpose(xnp2)) / float(batch_size)
    grad_Ws = np.matmul(Ws, (grad_Ws + np.transpose(grad_Ws)))
    grad_Ws = grad_Ws
    print grad_Ws
    Ws = Ws - lr_slow * grad_Ws

    loss = 0.5 * np.mean(np.square(y2 - np.matmul(Wf, np.matmul(Ws, xnp2)))) # / float(batch_size)
