...
tf.flags.DEFINE_string("train_image_dir", "/media/DATA/MS-COCO/train2014/",
                       "Training image directory.")
tf.flags.DEFINE_string("val_image_dir", "/media/DATA/MS-COCO/val2014",
                       "Validation image directory.")

tf.flags.DEFINE_string("train_captions_file", "/media/DATA/MS-COCO/annotations/captions_train2014.json",
                       "Training captions JSON file.")
tf.flags.DEFINE_string("val_captions_file", "/media/DATA/MS-COCO/annotations/captions_val2014.json",
                       "Validation captions JSON file.")

tf.flags.DEFINE_string("output_dir", "/media/DATA/MS-COCO/tf-data", "Output data directory.")
...
tf.flags.DEFINE_integer("max_im_count", 1280,
...
tf.flags.DEFINE_string("word_counts_output_file", "/media/DATA/MS-COCO/word_counts.txt",
...
def _load_and_process_metadata(captions_file, image_dir):
  ...
  # Extract the filenames, length 80k list with (id,fname) tuples as elements
  ...
  for annotation in caption_data["annotations"]:
    ...
    id_to_captions[image_id].append(caption)  # dict, int(id) to caption list
  ...
  image_metadata = []
  num_captions = 0
  log.info('proceed %d images!' % min(FLAGS.max_im_count, len(id_to_filename)))
  for image_id, base_filename in id_to_filename[0:FLAGS.max_im_count]:
    if num_captions % 100 == 0:
      log.info('%d images processed' % num_captions)
...
def main(unused_argv):
  ...
  # both train and val dataset
  # length 1280 list, each ele is a namedtuple [img_id, full_img_path, caption_list (len=5)]
  # in caption list, each elem is a list like [<s>, word1, ... wordn, </s>]
  ...
  # vocab._vocab: a dictionary mapping raw words to id
  # if not exists, mapping to 'unk'
